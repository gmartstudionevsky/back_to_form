...
export const BottomTabs = () => {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-20 border-t border-slate-200 bg-white/95 shadow-[0_-8px_24px_rgba(15,23,42,0.08)] backdrop-blur safe-bottom">
      <div className="mx-auto grid max-w-3xl grid-cols-5 gap-2 px-3 py-2">
        {tabs.map(tab => (
          <NavLink
            key={tab.path}
            to={tab.path}
            ...
            className={({ isActive }) =>
              `flex min-w-0 items-center justify-center rounded-xl px-2 py-2 text-[11px] font-semibold transition sm:text-xs ${
                isActive
